...
* Added 'quota=' script argument to restore the original 'literal' behavior of
  dymamic quotas instead of the new default 'stable' mode.
...
const byte VERS_MAJ = 1;
const byte VERS_MIN = 5;
const byte VERS_REV = 0;
const string VERS_UPD = "2016-05-24";
...
void ScanGrids()
{
	...
	while (b-- > 0) {
		...
		match = tagRegex.Match(blocks[b].CustomName);
		if (match.Success) {
...
void ScanBlocks<T>()
{
	...
	blocks = new List<IMyTerminalBlock>();
	GridTerminalSystem.GetBlocksOfType<T>(blocks, blk => (gridDocked.ContainsKey(blk.CubeGrid) == false || gridDocked[blk.CubeGrid] == true));
	b = blocks.Count;
	while (b-- > 0) {
		block = blocks[b];
...
void ParseBlockTags()
{
	...
	string[] attrs, fields;
	int a, i, x, y, priority, spanwide, spantall;
	long amount;
	float ratio;
	bool force, err;
	StringBuilder name;
	...
	while (enumBlock.MoveNext()) {
		...
		name.Clear();
		name.Append(blockname, 0, match.Index);
		name.Append(tagOpen);
		if (tagPrefix != "")
		...
		for (a = 0;  a < attrs.Length;  a++) {
			...
			if (attr.Length >= 4 & "LOCKED".StartsWith(attr, StringComparison.OrdinalIgnoreCase)) {
				...
				while (i-- > 0)
					invenLocked.Add(block.GetInventory(i));
				name.Append("LOCKED ");
			} else if (attr.Equals("EXEMPT", StringComparison.OrdinalIgnoreCase)) { // for AIS compatibility
				i = block.GetInventoryCount();
				while (i-- > 0)
					invenLocked.Add(block.GetInventory(i));
				name.Append("EXEMPT ");
...
{
	int l, x, y, wide, spanwide, spantall, height, priority;
	long amount, round;
	float ratio, scale;
	bool force;
	...
	types.ForEach((string qtype) => {
		...
		if (panel != null) {
			...
			for (y = 0;  y < spantall;  y++) {
				...
				for (x = 0;  x < spanwide;  x++) {
					spanLines[x] = empty;
					slim = panel.CubeGrid.GetCubeBlock(new Vector3I(panel.Position  +  x * wide * matrix.Right  +  y * matrix.Down));
		...
		if (FRACTIONAL_TYPES.Contains(qtype) == false)
			round = 1000000L;
		if (quotaStable & typeAmount[qtype] > 0L) { // TODO
			scalesubs.Clear();
...
void AddInvenRequest(IMyInventory inven, string itype, string isub, int priority, long amount)
{
	...
	if ((inven.Owner is IMyOxygenGenerator | inven.Owner is IMyReactor | inven.Owner is IMyRefinery | inven.Owner is IMyUserControllableGun) & ((inven.Owner is IMyLargeInteriorTurret) == false))
		(inven.Owner as IMyTerminalBlock).SetUseConveyorSystem(false);
} // AddInvenRequest()
...
long TransferItem(string itype, string isub, long amount, IMyInventory fromInven, IMyInventory toInven)
{
	...
	uint id;
	int? index;
	string stype, ssub, blocktype, blocksub;
...
{
	int spanwide, spantall, rows, wide, width, height;
	int x, y, r, w;
	float fontsize;
	string[][] spanLines;
	string text;
...
void Main(string argument)
{
	...
	int i, j, argCycle, unused, runtime;
	bool argRewriteTags, argScanCollectors, argScanDrills, argScanGrinders, argScanWelders, argQuotaStable, argTestComplexity, toggle;
	...
	blocks = new List<IMyTerminalBlock>();
	GridTerminalSystem.GetBlocksOfType<IMyProgrammableBlock>(blocks, block => (block == Me) | tagRegex.IsMatch(block.CustomName));
	i = blocks.IndexOf(Me);
	...
	if (tagRegex.IsMatch(Me.CustomName)) {
		Me.SetCustomName(tagRegex.Replace(Me.CustomName, msg, 1));
	} else {
		Me.SetCustomName(Me.CustomName + " " + msg);
	}
	if (i != j) {
	...
	if (panelFiller == "" & numCalls > cycleLength) {
		msg = " X_\0 I}\0 > 4\0 5HDY\0 .H77+)&C\0 'H3J6&$#*/\0 !G1D\"1R,\0zI.G*,+#0\"=(\0uI,7.+*)'$'&$7/)\0pJ)</-*$''&;#&$\"0&\0lL(=/3%$*%&,\"+%'#&.(\0h:;$&50,&+*&$*%*$'%%#%&,\0d9=$&72++&$-$()#''%$$&%(#&\0a8?&!;6,2,>(%#%&$('&\0]8.,+A6'#)+*>)&\"$3'*\0Z9/*-0)1/3(#\"(*)'+(#!5\"%&&\0W80+-.37)/-+'(%,&#\"=('\0T8&&$-,-=5',.,(%44\"1')\0R7((%),.G0%*/*9\"+-#5((\0O8(*(#.,./0:*$\"-7\";\"'1''\0M7&-;)-<&*)(01/*5%'3'&\0K6'*A&(W'&($&/;$-)&4'&\0H8&&1\"\"$.%\"C&(()$%)7F#'+!!'.''\0F8%%(%'\"7I)$'$&\"##,;?&&'3(('\0E9#$('+*,+.&'#*$#$'*''&8*\"7\">'''\0C:##(%.,+,/%C.',%CJ%$'&)\0A;,&-!$*+%5'&\">,)-!F8%$+%&&*\0@=)$)(&'-'4-+'4()*\".$$$/-:(%&*\0>?'#**;'%\"&\"%+),%&+'(/&$%$#1+6,$&,\0=?&#,)3$-'$\"%+.%:)*,#'%!$5**\"$0#&-\0<@/-2'>.A7(;$54'!%%'%.\0;A.//(*&.38>($$1$63-'%%/\0:B/..(*/&*!-(I(#)-$6+/#%&#&0\09C1&3(+-(*#U'\"'&(*$6+1%+%1\08F/,-',*!#%`*&%7#6+1+&%2\07G)#\"/,&,-$8&83)'%35*2-%$4\06J)5*&,,8$(26(B3,1,&#6\06L*3*&-*E03'\"\"B3,%#).#%6\05N2,*%,+>J%!\"Y&%$-,#%8\05P0/''+..$&&,6'=(W'&$9\04S.?)&'+!'$(/0'2X5&%$<\04V:23B0,%*r'$$%=\04Y92,%%0$-/4 !\"%@\04\\;.(#)\"@$.-;3$=:B\04`<(2%C#-(=a+F\04c:$3,$-#&6&0#80$+&2%H\04 !2,%*).-%,/24-1&J\04>%D#52,%)'%$+'$4,%#.%&9*P\04<&)N$(#'I'(($F$(&0).S\05A\\$\"#&.$:('))C#OU\054!&c\"#!),%;(&+,h]\060:E=+%0$#$#(&$\"&1Vf\06/1[0*&1$%\"%'%%$(@$'\"{\07,-a/+%0$&%$'%%#+ E\08*)g.*&0#&&%&&&\", B\09z.*%1#%'%&%&#, @\0:z-*&.&#)%%%'#, >\0;y.*'+&#*%%%'%* =\0<x/*&,%#.+'%) =\0=x0('/%%*('&) <\0>x/)'-'$+&(&( <\0@v/*&,(%##%%(&' ;\0Av/*'+'+%$)'& :\0Cu/*&+()(\")'( 6\0Et.+&+'*0)) 3\0Ft-+'+')0() 3\0Hr.+&,&)0(( 2\0Kp-+&,'&1)( /\0Mn,-%-&#3*& /\0Ol+.%,&#3*' -\0Ri+-',$#3+' *\0Tg,,'+%#2-$ *\0Wd+,(+#%)#$0# '\0Za+*)3(%# 8\0]^**)2*%$ 4\0aY**)2) ;\0dV)+(2( :\0hR(*)2& 9\0lN'))*#'$ 7\0pJ%)*'$ A\0uC%/%&# ?\0z=$0&%# ;\0 !7!3$%# 7\0 'K# 4\0 . T\0 5 F\0 > 4\0 I}\0 X_\0";
		sb.Clear();
		j = 0;
		for (i = 0;  i < msg.Length;  i++) {
			if (msg[i] == '\0') {
				...
				sb.Append(new String((char)(' '+j*('I'-' ')),(int)msg[i]-32));
				j = 1 - j;
			}
		}
		panelFiller = sb.ToString();
	}
